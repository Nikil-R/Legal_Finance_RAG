...
class QuestionResult:
    ...
    def passed(self) -> bool:
        ...
        return (
            self.success and
            self.faithfulness_score >= 0.7 and
            self.correctness_score >= 0.6 and
            self.citation_quality_score >= 0.8 and
            self.retrieval_relevance_score >= 0.5
        )
...
class RAGEvaluator:
    ...
    def evaluate_question(self, question: GoldenQuestion) -> QuestionResult:
        ...
        try:
            ...
            faith_result = self.faithfulness_scorer.score(
                ...
                context=result.context or self._build_context_from_sources(result.sources),
    ...
    def evaluate_all(
        ...
        avg_faith = sum(r.faithfulness_score for r in successful_results) / len(successful_results) if successful_results else 0.0
        avg_correct = sum(r.correctness_score for r in successful_results) / len(successful_results) if successful_results else 0.0
        avg_citation = sum(r.citation_quality_score for r in successful_results) / len(successful_results) if successful_results else 0.0
        avg_retrieval = sum(r.retrieval_relevance_score for r in successful_results) / len(successful_results) if successful_results else 0.0
        avg_overall = sum(r.overall_score for r in successful_results) / len(successful_results) if successful_results else 0.0
        ...
        for domain in ["tax", "finance", "legal"]:
            ...
            if domain_results:
                domain_scores[domain] = {
                    ...
                    "avg_score": sum(r.overall_score for r in domain_results) / len(domain_results),
                    "pass_rate": len([r for r in domain_results if r.passed]) / len(domain_results)
                }
        ...
        for difficulty in ["easy", "medium", "hard"]:
            ...
            if diff_results:
                difficulty_scores[difficulty] = {
                    ...
                    "pass_rate": len([r for r in diff_results if r.passed]) / len(diff_results)
                }
