...
from evaluation.evaluator import EvaluationResult, QuestionResult
...
class EvaluationReporter:
    ...
    def generate_console_report(self, result: EvaluationResult) -> str:
        ...
        lines.append(f"Overall Status:   {status_icon} {'PASSED' if result.passed else 'FAILED'}")
        lines.append(f"Pass Rate:        {result.pass_rate:.1%} ({result.questions_passed}/{result.total_questions})")
        lines.append("")
        ...
        lines.append("-" * 70)
        lines.append(f"  Faithfulness:       {result.avg_faithfulness:.3f}  {'✓' if result.avg_faithfulness >= 0.7 else '✗'}")
        lines.append(f"  Correctness:        {result.avg_correctness:.3f}  {'✓' if result.avg_correctness >= 0.6 else '✗'}")
        lines.append(f"  Citation Quality:   {result.avg_citation_quality:.3f}  {'✓' if result.avg_citation_quality >= 0.8 else '✗'}")
        lines.append(f"  Retrieval Relevance:{result.avg_retrieval_relevance:.3f}  {'✓' if result.avg_retrieval_relevance >= 0.5 else '✗'}")
        ...
        for domain, scores in result.domain_scores.items():
            lines.append(f"  {domain.upper():12} | Questions: {scores['count']:2} | Avg: {scores['avg_score']:.3f} | Pass: {scores['pass_rate']:.1%}")
        lines.append("")
        ...
        for diff, scores in result.difficulty_scores.items():
            lines.append(f"  {diff.upper():12} | Questions: {scores['count']:2} | Avg: {scores['avg_score']:.3f} | Pass: {scores['pass_rate']:.1%}")
        lines.append("")
        ...
        if failed:
            ...
            for r in failed[:10]:  # Show top 10
                ...
                lines.append(f"    Faith: {r.faithfulness_score:.2f} | Correct: {r.correctness_score:.2f} | Citation: {r.citation_quality_score:.2f} | Retrieval: {r.retrieval_relevance_score:.2f}")
                if r.error:
