...
from app.api.dependencies import get_retriever, clear_pipeline_cache
...
async def ingest_documents(request: IngestRequest) -> IngestResponse:
    ...
    try:
        ...
        logger.info("Ingestion complete | Chunks: %d | Time: %.2fs", result['chunks_stored'], elapsed)
...
async def list_domains() -> dict:
    ...
    return {
        "domains": [
            {"id": "tax", "name": "Tax Laws", "description": "Indian income tax, GST, and related tax legislation"},
            {"id": "finance", "name": "Financial Regulations", "description": "RBI guidelines, banking regulations, SEBI rules"},
            {"id": "legal", "name": "Legal Provisions", "description": "Contract Act, Companies Act, general legal provisions"},
            {"id": "all", "name": "All Domains", "description": "Search across all domains"}
        ]
    }
